...
def get_NS(data_dir, num_train, num_test, task, missing_rate):
    """
    Args:
    ...
    train_au  = torch.tensor(data[:50,   ::1, ::1, :num_train], dtype=torch.float).transpose(0, 3).reshape(num_train, -1, 50)
    test_au   = torch.tensor(data[:50,   ::1, ::1, -num_test:], dtype=torch.float).transpose(0, 3).reshape(num_test,  -1, 50)
