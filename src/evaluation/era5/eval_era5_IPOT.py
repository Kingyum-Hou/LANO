...
#model_path = 'save/ERA5/IPOT/task3_mr=5/best-v1.ckpt'
#model_path = 'save/ERA5/IPOT/task3_mr=25/best.ckpt'
model_path = 'save/ERA5/IPOT/task3_mr=50/best-v2.ckpt'
...
test_pos_pred = torch.rand(16200, 2).to(device)
test_To       = 7
...
with torch.no_grad():
    for batch_idx, batch in enumerate(test_dataloader):
        ...
        pos_pred = pos[0]
        aPos_had = torch.concat([x_had, pos_had], dim=-1)
        ...
        if plot_flag:
            ...
            for i in range(7):
                ...
                axes[2, i].axis('off')
            plt.show()
            plt.savefig(f'imgs/IPOT_{batch_idx}.png')
            plt.clf()
            #plot_flag = False
print('ok!')
