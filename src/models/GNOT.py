...
from models.nn_module4GNOT.Basic import MLP, LinearAttention, ACTIVATION
from models.nn_module4GNOT.Embedding import timestep_embedding, unified_pos_embedding
...
class Model(nn.Module):
    ...
    def __init__(self, args, n_experts=3):
        ...
        if args.unified_pos and args.geotype != 'unstructured':  # only for structured mesh
            self.pos = unified_pos_embedding(args.shapelist, args.gnot_ref)
            self.preprocess_x = MLP(args.gnot_ref ** len(args.shapelist), args.n_hidden * 2,
                                    args.n_hidden, n_layers=0, res=False, act=args.act)
            self.preprocess_z = MLP(args.fun_dim + args.gnot_ref ** len(args.shapelist), args.n_hidden * 2,
                                    args.n_hidden, n_layers=0, res=False, act=args.act)
        else:
    ...
    def forward(self, x, fx, x_fx, T=None, geo=None):
        pos = x
        if self.args.unified_pos:
            x = self.pos.repeat(x.shape[0], 1, 1)
        if fx is not None:
            fx = torch.cat((x_fx, fx), -1)
