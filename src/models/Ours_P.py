...
class PhLP(nn.Module):
    ...
    def forward(self, y, mask):
        ...
        return y_out, next_mask, score_inner
...
class KernelIntegrator(nn.Module):
    ...
    def forward(self, y, mask):
        ...
        y_in_, new_mask, score_inner = self.phlp(self.ln_1(y_in), mask)
        y_out = y_in_ + y_in
        ...
        return y_out, new_mask, score_inner
...
class OursLNOScoreModel(nn.Module):
    ...
    def forward(self, pos, x, mask):
        ...
        for _, block in enumerate(self.kernelProcessor):
            y, mask, score_inner = block(y, mask)
            score_inner_list.append(score_inner) 
            mask_list.append(mask)
        x  = self.projector(y)
        return x, score_inner_list, mask_list
