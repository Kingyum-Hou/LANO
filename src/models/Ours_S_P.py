...
class PhLP(nn.Module):
    ...
    def forward(self, pos, y, mask):
        ...
        y_out, score = self.phca_decoder(x_out, z)
        return y_out, new_mask, score
...
class KernelIntegrator(nn.Module):
    ...
    def forward(self, pos, y, mask):
        ...
        # PhLP
        y_in_, new_mask, score = self.phlp(pos, self.ln_1(y_in), mask)
        y_out  = y_in_ + y_in
        # mlp
        y_out = self.mlp(self.ln_2(y_out)) + y_out
        return y_out, new_mask, score, y_in_ 
...
class OursLNOModel(nn.Module):
    ...
    def forward(self, pos, x, mask):
        ...
        for _, block in enumerate(self.kernelProcessor):
            y, mask, score, y_inside = block(pos, y, mask)
            score_list.append(score)
            mask_list.append(mask)
            y_list.append(y_inside)
        x  = self.projector(y)
        return x, score_list, mask_list, y_list
