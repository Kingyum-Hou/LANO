nohup python -u src/main.py \
    ...
    >> logs/NSv-5_task3_OFORMER_baseline_mr=50_original_test.log 2>&1 & sleep 5s &
...
nohup python -u src/main.py \
    ...
    callback.early_stopping.monitor=train/full_loss \
    tag=baseline_original \
    >> logs/NSv-5_task3_OFORMER_baseline_mr=25_original_test.log 2>&1 & sleep 5s &
...
nohup python -u src/main.py \
    ...
    callback.early_stopping.monitor=train/full_loss \
    tag=baseline_original \
    >> logs/NSv-5_task3_OFORMER_baseline_mr=5_original_test.log 2>&1 & sleep 5s
